...
public class BoardAPIController {
    ...
    public String addBoard(@PathVariable Long boardQuantity, HttpServletRequest request) throws Exception {
        ...
        System.out.println("authorityName = " + authorityName == RoleStatus.ROLE_ADMIN.name());
        System.out.println("authorityName = " + authorityName.equals(RoleStatus.ROLE_ADMIN.name()));
        System.out.println("RoleStatus.ROLE_ADMIN.name() = " + RoleStatus.ROLE_ADMIN.name());
    ...
    /*
    @PostMapping("/image/finalId")
    @ApiOperation("board의 마지막 Id를 가져오는 코드")
    @ResponseBody
    @Trace
    public FinalBoardIdDto finalApi() throws Exception {
        FinalBoardIdDto finalBoardIdDto = new FinalBoardIdDto(boardService.findFinalBoard());
        return finalBoardIdDto;
    }
